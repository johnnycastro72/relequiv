...
class Fallares
{
    ...
    public function contarResultados($idPrueba, $idModelo)
    {
        ...
        $sql="select count(distinct(idAprendiz)) from tfallares where idPrueba = ? and idModelo = ?";
    ...
    public function contarFase($idPrueba, $idFase, $idAprendiz)
    {
        $con = new conectar();
        $this->Conexion = $con->getConnection();
        $sql="select count(idResultado) from tfallares where idPrueba = ? and idFase = ? and idAprendiz = ?";
        $resultado = $this->Conexion->prepare($sql);
    ...
    public function tiempoEvaluacion($idAprendiz, $idFase)
    {
        $con = new conectar();
        $this->Conexion = $con->getConnection();
        $sql="select sum(Tiempo) from tfallares where idFase = ? and idAprendiz = ?";
        $resultado = $this->Conexion->prepare($sql);
    ...
    public function aciertos($idAprendiz, $idFase)
    {
        $con = new conectar();
        $this->Conexion = $con->getConnection();
        $sql="select count(*) from tfallares where idFase = ? and idAprendiz = ? and Acierto=1";
        $resultado = $this->Conexion->prepare($sql);
		$resultado->execute([$idFase, $idAprendiz]);
        $count = $resultado->fetchColumn();
		return $count;
    ...
    public function errores($idAprendiz, $idFase)
    {
        $con = new conectar();
        $this->Conexion = $con->getConnection();
        $sql="select count(*) from tfallares where idFase = ? and idAprendiz = ? and Acierto=0";
        $resultado = $this->Conexion->prepare($sql);
		$resultado->execute([$idFase, $idAprendiz]);
        $count = $resultado->fetchColumn();
		return $count;
    ...
    public function acertoResultado($idPrueba, $idFase, $idAprendiz, $Preguntas)
    {
        $con = new conectar();
        $this->Conexion = $con->getConnection();
        $sql="SELECT COUNT(totals.Acierto) AS falla FROM tfallares, (SELECT * FROM tfallares WHERE idPrueba = '$idPrueba' AND idFase = '$idFase' AND idAprendiz = '$idAprendiz' ORDER BY idResultado DESC LIMIT $Preguntas) totals WHERE tfallares.idResultado = totals.idResultado AND totals.Acierto = 0";
	...
	{	
        $con = new conectar();
        $this->Conexion = $con->getConnection();
		$sql="insert into tfallares (idAprendiz, idPregunta, Acierto, Tiempo, Intento, idPrueba, idModelo, idFase) values ('$this->idAprendiz', '$this->idPregunta', '$this->Acierto', '$this->Tiempo', '$this->Intento', '$this->idPrueba', '$this->idModelo', '$this->idFase')";
		$resultado=$this->Conexion->query($sql);
		return $resultado;	
	}
    ...
    public function borrarResultados($idPrueba)
    {
        $con = new conectar();
        $this->Conexion = $con->getConnection();
        $sql="delete from tfallares where idPrueba=" . $idPrueba;
        $resultado = $this->Conexion->query($sql);
		return $resultado;
    }
    ...
    public function consultaResultados($idAprendiz)
	{
        $con = new conectar();
        $this->Conexion = $con->getConnection();
		$sql="select * from tfallares where idAprendiz='$idAprendiz'";
		$resultado=$this->Conexion->query($sql);
		return $resultado;	
	}
    ...
    public function sumaTiempo($idAprendiz)
    {
        $con = new conectar();
        $this->Conexion = $con->getConnection();
        $sql="select sum(Tiempo) from tfallares where idAprendiz = ?";
        $resultado = $this->Conexion->prepare($sql);
